...
## 1. LUKS passphrase is a shared, hardcoded placeholder

`autoinstall.yaml.template`'s `storage.layout.password` is currently just
`"changeme"` — the same value baked into every ISO, not treated as a real
secret. [`README.md`](README.md#update-a-luks-passphrase) documents
rotating it manually via `cryptsetup luksChangeKey` after first boot, but
nothing enforces or automates that, so a machine can be left running
indefinitely on the shared default.
